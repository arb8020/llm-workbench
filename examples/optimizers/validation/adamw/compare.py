#!/usr/bin/env python3
"""
AdamW optimizer comparison script.

This script compares AdamW implementations (skeleton vs solution) against
PyTorch reference implementation across multiple test surfaces.

Usage:
    python examples/optimizers/validation/adamw/compare.py --mode skeleton
...
def load_adamw_implementation(mode):
    """Load AdamW implementation based on mode."""
    if mode == "skeleton":
        try:
            from skeleton import adamw_update, adamw_init, AdamWState
            print("✅ Successfully imported from skeleton")
            return adamw_update, adamw_init, AdamWState
        ...
        try:
            from solution import adamw_update, adamw_init, AdamWState
...
def get_reference_trajectories(
    ...
    """Get reference trajectory from PyTorch AdamW"""
...
def test_adamw_on_surface(adamw_update, adamw_init, surface_name: str, surface_fn, grad_fn, initial_params: jnp.ndarray, lr: float = 0.01, weight_decay: float = 0.01):
    ...
    torch_traj = get_reference_trajectories(
        initial_params, lr, surface_fn, grad_fn, weight_decay, num_steps
    )
...
def compare_adamw_implementations(adamw_update, adamw_init, AdamWState, mode_name):
    """Run full AdamW test suite"""
    print(f"\n🧪 Testing AdamW {mode_name} implementation")
    print("="*60)
...
def main():
    parser = argparse.ArgumentParser(description="Compare AdamW optimizer implementations")
    parser.add_argument("--mode", choices=["skeleton", "solution"], required=True,
                       help="Which implementation to test")
    args = parser.parse_args()
    
    print("🚀 AdamW Optimizer Comparison Suite")
    print(f"Mode: {args.mode}")
    
    # Load implementation
    adamw_update, adamw_init, AdamWState = load_adamw_implementation(args.mode)
    
    # Run comparison tests
    results = compare_adamw_implementations(adamw_update, adamw_init, AdamWState, args.mode)
